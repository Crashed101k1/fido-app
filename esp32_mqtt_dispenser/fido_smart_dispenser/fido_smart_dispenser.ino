...
void manejarComandosMQTT(String command, JsonObject data) {
  ...
  publishResponseWithType("dispense", "started", "Dispensación iniciada", "manual");
    ...
    if (data.containsKey("schedules") && data["schedules"].is<JsonArray>()) {
      ...
      for (JsonVariant scheduleItem : schedulesArray) {
        ...
        if (schedule.containsKey("hour") && schedule.containsKey("minute") && 
            schedule.containsKey("portion") && schedule.containsKey("active")) {
          
          horarios[numHorarios].hora = schedule["hour"];
          horarios[numHorarios].minuto = schedule["minute"];  
          horarios[numHorarios].porcion = schedule["portion"];
          horarios[numHorarios].activo = schedule["active"];
          horarios[numHorarios].ultimaEjecucion = 0;
          
          Serial.printf("[HORARIOS] Horario %d: %02d:%02d - %.1fg - %s\n", 
                       numHorarios + 1, 
    ...
    if (data.containsKey("schedules") && data["schedules"].is<JsonArray>()) {
      ...
      for (JsonVariant scheduleItem : schedulesArray) {
        ...
        if (schedule.containsKey("hour")) hora = schedule["hour"];
        if (schedule.containsKey("minute")) minuto = schedule["minute"];
        if (schedule.containsKey("portion")) porcion = schedule["portion"];
        if (schedule.containsKey("active")) activo = schedule["active"];
...
void iniciarDispensacion() {
  ...
  publishResponseWithType("dispense", "completed", mensajeCompleto.c_str(), tipoDispensacion);
}
...
void loop() {
  ...
  if (isConnected() && millis() - lastSensorUpdate >= SENSOR_UPDATE_INTERVAL) {
    ...
    if (millis() - lastOfflineLog >= 10000) { // Log cada 10 segundos si está desconectado
